  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-white/60 mt-1">All form submissions from the website.</p>
      </header>
            ...
            <tbody>
              {items.map((c) => (
                <tr key={c.id} className={`border-t border-white/5 ${!c.isRead ? 'bg-brand-500/5' : ''}`}>
                  <td className="px-4 py-3 text-xs text-white/60 whitespace-nowrap">{c.createdAt.toLocaleString('en-IN')}</td>
                  <td className="px-4 py-3 font-medium">
                    {c.name} {c.age && <span className="text-white/40">({c.age})</span>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-white/70">{c.program ?? '—'}</td>
                  <td className="px-4 py-3 max-w-sm text-white/70">{c.message ?? '—'}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      {!c.isRead && (
                        <form action={markRead}>
                          <input type="hidden" name="id" value={c.id} />
                          <button type="submit" className="text-xs text-white/70 hover:text-brand-400">Mark read</button>
                        </form>
                      )}
