...
export default async function TeamAdminPage() {
  ...
  return (
    <AdminListShell title="Team" newHref="/nvx-panel-7q2/team/new" newLabel="Add member">
      {team.length === 0 ? (
          ...
          {team.map((m) => (
            <li key={m.id} className="flex items-center justify-between border-b border-white/5 px-4 py-3 last:border-0">
              ...
              <div className="flex items-center gap-4">
                <Link href={`/nvx-panel-7q2/team/${m.id}`} className="text-xs text-brand-400 hover:underline">Edit</Link>
