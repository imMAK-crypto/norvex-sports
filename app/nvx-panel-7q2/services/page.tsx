...
export default async function ServicesAdminPage() {
  ...
  return (
    <AdminListShell title="Services" newHref="/nvx-panel-7q2/services/new" newLabel="New service">
      {services.length === 0 ? (
          ...
          {services.map((s) => (
            <li key={s.id} className="flex items-center justify-between border-b border-white/5 px-4 py-3 last:border-0">
              ...
              <div className="flex items-center gap-4">
                <Link href={`/nvx-panel-7q2/services/${s.id}`} className="text-xs text-brand-400 hover:underline">Edit</Link>
