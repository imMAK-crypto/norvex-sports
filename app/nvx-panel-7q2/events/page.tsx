...
export default async function EventsAdminPage() {
  ...
  return (
    <AdminListShell title="Events" newHref="/nvx-panel-7q2/events/new" newLabel="New event">
      {events.length === 0 ? (
          ...
          {events.map((e) => (
            <li key={e.id} className="flex items-center justify-between border-b border-white/5 px-4 py-3 last:border-0">
              ...
              <div className="flex items-center gap-4">
                <Link href={`/nvx-panel-7q2/events/${e.id}`} className="text-xs text-brand-400 hover:underline">Edit</Link>
